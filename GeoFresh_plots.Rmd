...
# Create histograms for topography_local_filtered and topography_upstream_filtered
```{r}
# topography_local_filtered
par(mfrow = c(1, 2))  
...
soil_local_filtered <- soil_local %>%
  select(awcts_mean,wwp_mean,orcdrc_mean,phihox_mean,bldfie_mean,cecsol_mean,crfvol_mean,acdwrb_mean,bdricm_mean,bdrlog_mean,histpr_mean,slgwrb_mean)
...
```{r}
# filter environmental_variables by Soil
soil_variables <- filter(environmental_variables, Category == "Soil")
# copy column names from soil_local_filtered
soil_column_names <- names(soil_local_filtered)

# remove "_mean"
soil_column_names_split <- sapply(
  soil_column_names, 
  function(x) strsplit(x, "_")[[1]][1],
  USE.NAMES = FALSE
  )
# create new subset without mean 
soil_variables_subset <- subset(soil_variables, Abbreviation %in% soil_column_names_split) 
# create a new data frame with only Abbreviation and Variable
soil_variables_subset_filtered <- soil_variables_subset %>% select('Abbreviation', 'Variable')
                                  soil_variables_subset %>% select(c('Abbreviation', 'Variable'))

 
soil_local_filtered_norm <- data.frame(
ID = 1:100,
awcts_mean = rnorm(100),wwp_mean = rnorm(100),orcdrc_mean = rnorm(100),phihox_mean = rnorm(100),bldfie_mean = rnorm(100),cecsol_mean = rnorm(100),
crfvol_mean = rnorm(100),acdwrb_mean = rnorm(100),bdricm_mean = rnorm(100),bdrlog_mean = rnorm(100),histpr_mean = rnorm(100),slgwrb_mean = rnorm(100))
...
variable_names <- deframe(soil_variables_subset_filtered)
...
for (col_name in names(soil_local_filtered)) {
  hist(soil_upstream_filtered[[col_name]],
       main = paste("Histogram for", col_name),
       xlab = col_name,
       col = "skyblue",
       border = "black")
...
```{r}
# soil_upstream_filtered
par(mfrow = c(1, 2))  

for (col_name in names(soil_upstream_filtered)) {
  hist(soil_upstream_filtered[[col_name]],
       main = paste("Histogram for", col_name),
       xlab = col_name,
       col = "skyblue",
